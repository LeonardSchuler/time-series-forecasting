...
def to_train_validation_test_data(
    df: pd.DataFrame, train_end_date: str, validation_end_date: str
):
    """
    ...
    # Logging the results
    logger.info(f"# of training observations: {n_train} | {(n_train / n * 100):.2f}%")
    logger.info(f"# of validation observations: {n_val} | {(n_val / n * 100):.2f}%")
    logger.info(f"# of test observations: {n_test} | {(n_test / n * 100):.2f}%")
